...
Answer 1: One of the most important things for code reviews is to have good names for variables, functions, classes, and such. The better the names are, the more everything makes sense to people who didn't write the code. The code might work, but it's difficult to understand. Good names are descriptive -- "person_1" is better than "x1" if you're talking about a person. It's also important to have good comments explaining what's going on in your code, again saving a lot of time for code readers. People should be able to run a function without reading the code, just the comment. 

Answer 2: Using containers like with Docker makes these types of processes easier, things like server administration. Containers are a form of virtualization, kind of like virtual environments in that you're creating a virtual place to make sure your code can run with servers and other hardware that is potentially running something different or incompatible with your machine (in the same way as I can run Windows on a Mac as a virtual machine). The container is a virtualized software developement platform and acts like a translator in this case, so you don't have to manually configure everything and risk either making an error or redoing it every time you want to work with a different server. The container just contains the bare minimum code required to run applications, so it's lightweight as well. 
