...
"""
def mapping(nums, discount):
    # take in a list of numbers and a string like "50%"
    # return a list where each number is "discount percent" of the original
    disc_coef = int(discount[0] + discount[1])
    for i in range(len(nums)):
        nums[i] *= (disc_coef / 100)
        if nums[i] % 1 == 0:
            nums[i] = int(nums[i])  
    return nums
    
    
print(mapping([2, 4, 6, 11], "50%"))
print(mapping([10, 20, 40, 80], "75%"))
print(mapping([100], "45%"))
